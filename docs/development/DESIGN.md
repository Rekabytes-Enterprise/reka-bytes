...
export function useTypography() {
  ...
  const ty = (...tokens: (keyof typeof typeStyles)[] | [string]) => cn(...(tokens as string[]));
  return { t, ty };
}
```
...
1. **Hero** — 8/4 split: left = `displayXL` headline ("Learn to vibe code _properly_."), mono sub-line, CTA pair, seats-left chip; right = full-height WebGL byte-stream canvas bleeding to viewport edge.
